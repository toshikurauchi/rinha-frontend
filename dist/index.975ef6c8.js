...
})({"6Jor2":[function(require,module,exports) {
...
function renderPage(pageIdx) {
    ...
    for(let i = startIdx; i < endIdx; i++)renderLine(lines[i], contentContainer);
...
function renderPagination(pageIdx) {
    ...
    if (pageIdx > 0) {
        createPageLink("\xab", 0, pageIdx, paginationContainer, true);
    ...
    if (totalPages - pageIdx < pagesInEachSide) startIdx = Math.max(0, totalPages - SHOW_PAGE_NUMBERS);
    const lastIdx = Math.min(totalPages - 2, startIdx + SHOW_PAGE_NUMBERS - 1);
    if (startIdx > 1) createElement("span", null, paginationContainer, "...");
    for(let i = startIdx; i <= lastIdx; i++){
        if (i === 0) continue;
        createPageLink(i + 1, i, pageIdx, paginationContainer);
    }
    if (lastIdx < totalPages - 2) createElement("span", null, paginationContainer, "...");
    createPageLink(totalPages, totalPages - 1, pageIdx, paginationContainer);
    if (pageIdx < totalPages - 1) {
        createPageLink("\u203A", pageIdx + 1, pageIdx, paginationContainer, true);
        createPageLink("\xbb", totalPages - 1, pageIdx, paginationContainer, true);
    }
}
function createPageLink(text, pageIdx, currentPageIdx, parent, isSpecial) {
    if (pageIdx === currentPageIdx) return createElement("span", null, parent, text);
    else {
...
function addIndentation(lineElement, indentation) {
    for(let i = 0; i < indentation; i++)createElement("span", "indent", lineElement, INDENT);
}
...
function objToLines(obj, isArray, indentation) {
    ...
    for(const key in obj)if (obj.hasOwnProperty(key)) {
        ...
        if (value !== null && typeof value === "object") {
            ...
            lines.push(newLine(indentation, key, null, isArray, isNewArray ? "[" : null));
            const newLines = objToLines(value, isNewArray, indentation + 1);
            newLines.forEach((line)=>lines.push(line));
            if (isNewArray) lines.push(newLine(indentation, null, null, isArray, "]"));
        } else {
...
},{}]},["6Jor2","8lqZg"], "8lqZg", "parcelRequire9702")
